...
fn render_handoff_page(title: &str, message: &str, deep_link: &str) -> String {
    format!(
        "<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>SoundunCloud</title><style>:root{{color-scheme:light}}*{{box-sizing:border-box}}body{{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 18% 18%,rgba(255,113,52,.14),transparent 18%),radial-gradient(circle at 82% 14%,rgba(202,192,236,.32),transparent 22%),linear-gradient(180deg,#f7f3fb 0%,#fdfbfd 46%,#f2eef8 100%);color:#2f2442;font:15px/1.55 \"Segoe UI Variable Text\",\"Segoe UI\",Inter,system-ui,sans-serif}}main{{width:min(420px,calc(100vw - 40px));padding:14px}}.card{{border:1px solid #e8e0f3;border-radius:32px;background:rgba(255,255,255,.86);box-shadow:0 32px 100px rgba(188,177,220,.24);backdrop-filter:blur(20px);padding:32px;text-align:center}}.mark{{width:60px;height:60px;margin:0 auto 18px;border-radius:20px;background:#fff4ec;display:grid;place-items:center;box-shadow:0 14px 30px rgba(255,113,52,.14)}}.pulse{{width:16px;height:16px;border-radius:999px;border:2px solid rgba(255,106,26,.92);box-shadow:0 0 24px rgba(255,106,26,.2);position:relative}}.pulse::after{{content:\"\";position:absolute;inset:-8px;border-radius:999px;border:1px solid rgba(255,106,26,.22);animation:pulse 1.4s ease-out infinite}}.eyebrow{{margin:0 0 12px;color:#a194b8;font-size:11px;font-weight:700;letter-spacing:.24em;text-transform:uppercase}}h1{{margin:0;font-size:26px;line-height:1;letter-spacing:-.06em}}p{{margin:14px 0 22px;color:#7e7394}}a{{display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:0 18px;border-radius:999px;background:#ff5500;color:#fff;text-decoration:none;font-weight:700;box-shadow:0 16px 34px rgba(255,113,52,.24)}}@keyframes pulse{{0%{{transform:scale(.86);opacity:.85}}100%{{transform:scale(1.4);opacity:0}}}}</style></head><body><main><div class=\"card\"><div class=\"mark\"><div class=\"pulse\"></div></div><p class=\"eyebrow\">SoundunCloud</p><h1>{title}</h1><p>{message}</p><a href=\"{deep_link}\">Open app</a></div></main><script>const deeplink={deep_link:?};window.setTimeout(function(){{window.location.replace(deeplink);}},40);window.setTimeout(function(){{window.close();}},420);</script></body></html>"
    )
}
